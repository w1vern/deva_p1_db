...
class Project(Base):
    ...
    origin_file_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("files.id"), default=None, nullable=True)
    transcription_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("files.id"), default=None, nullable=True)
    ...
    origin_file: Mapped["File | None"] = relationship(
        lazy="selectin", foreign_keys=[origin_file_id])
    transcription: Mapped["File | None"] = relationship(
        lazy="selectin", foreign_keys=[transcription_id])
    summary: Mapped["File | None"] = relationship(
        lazy="selectin", foreign_keys=[summary_id])
